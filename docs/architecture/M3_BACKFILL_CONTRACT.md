...
Primary exhaustion pauses until reset. Secondary limits honor `Retry-After`, otherwise use a bounded backoff of at least one minute. The worker persists the installation/stage pause, preserves the cursor, and schedules a delayed physical wake for the same durable position under a timestamped execution key so it cannot collide with the active stately job. `401`, non-rate-limit `403`, and ambiguous `404` pause historical work and trigger one authoritative M2 installation inventory check; they do not hot-loop.
